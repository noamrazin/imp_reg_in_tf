...
from .experiment_result_factory import ExperimentResultFactory
...
class ExperimentBase(Experiment, ABC):
    """
    ...
    @abstractmethod
    def create_train_and_validation_evaluators(self, model: nn.Module, datamodule: DataModule, device, config: dict,
                                               state: dict, logger: logging.Logger) -> Tuple[TrainEvaluator, Evaluator]:
        """
    ...
    def create_trainer_callback(self, model: nn.Module, datamodule: DataModule, config: dict, state: dict, logger: logging.Logger) -> Callback:
        ...
        callbacks_dict = OrderedDict({ExperimentBase.LOGGING_CALLBACK_NAME: logging_callback})

        save_interval = config["save_every_num_val"] * config["validate_every"]
        if config["save_metric_plots"]:
            callbacks_dict[ExperimentBase.METRICS_PLOTTER_CALLBACK_NAME] = callbacks.MetricsPlotter(state["experiment_dir"],
        ...
        if config["save_checkpoints"]:
            ...
            callbacks_dict[ExperimentBase.CHECKPOINT_CALLBACK_NAME] = callbacks.Checkpoint(state["experiment_dir"],
        ...
        if config["early_stop"]:
            callbacks_dict[ExperimentBase.EARLY_STOPPING_CALLBACK_NAME] = callbacks.EarlyStopping(score_fn,
                                                                                                  score_name=config["score_metric_name"],
                                                                                                  largest=config["score_largest"],
    ...
    def create_experiment_result(self, model: nn.Module, datamodule: DataModule, trainer: Trainer, fit_output: FitOutput,
        ...
        if score_info.return_best_score:
            return ExperimentResultFactory.create_from_best_metric_score(metric_name=score_info.metric_name,
                                                                         fit_output=fit_output,
                                                                         largest=score_info.largest,
                                                                         is_train_metric=score_info.is_train_metric)

        return ExperimentResultFactory.create_from_last_metric_score(metric_name=score_info.metric_name,
                                                                     fit_output=fit_output,
                                                                     largest=score_info.largest,
                                                                     is_train_metric=score_info.is_train_metric)
    ...
    def cleanup(self, model: nn.Module, datamodule: DataModule, trainer: Trainer, fit_output: FitOutput, config: dict, state: dict,
        ...
        :return: ExperimentResult experiment result for the given fit output.
        """
        if isinstance(trainer.callback, callbacks.ComposeCallback) and ExperimentBase.WANDB_CALLBACK_NAME in trainer.callback.callbacks:
            wandb_callback = trainer.callback.callbacks[ExperimentBase.WANDB_CALLBACK_NAME]
    ...
    def run(self, config: dict, context: dict = None):
        ...
        train_evaluator, val_evaluator = self.create_train_and_validation_evaluators(model, datamodule, device, config, state, logger)
