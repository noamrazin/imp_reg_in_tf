...
class TrainBatchOutputEvaluator(TrainEvaluator):
    ...
    def __init__(self, metric_names, metric_tags=None):
        ...
        metrics_seq = [DummyAveragedMetric() for _ in metric_names]
        self.metric_infos = {metric_names[i]: MetricInfo(metric_names[i], metrics_seq[i], self.metric_tags[i]) for i in range(len(metric_names))}
        self.metrics = {metric_name: DummyAveragedMetric() for metric_name in metric_names}
