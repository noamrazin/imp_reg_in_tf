...
def predict_in_batches(model: nn.Module, dataset: torch.utils.data.Dataset, batch_size: int = 64,
                       log_every_num_batches: int = -1, device=get_device()) -> torch.Tensor:
    """
...
def iter_batch_predictions(model: nn.Module, dataset: torch.utils.data.Dataset, batch_size: int = 64, device=get_device()) -> torch.Tensor:
    """
...
def export_to_onnx(model: nn.Module, input_sizes: Sequence[int], output_path: str, input_names: Sequence[str] = None,
                   output_names: Sequence[str] = None, export_params: bool = True, verbose: bool = True, device=get_device()):
    """
