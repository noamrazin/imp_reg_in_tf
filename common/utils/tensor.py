...
def create_tensor_with_cp_rank(num_dim_per_mode, cp_rank: int, init_mean: float = 0.0, init_std: float = 1.,
                               fro_norm: float = 1, super_symmetric: bool = False) -> torch.Tensor:
    """
    ...
    :param super_symmetric: if True will return a super symmetric tensor
...
def convert_tensor_to_one_hot(tensor: torch.Tensor, num_options: int = -1):
    ...
    num_options = num_options if num_options > 0 else (flattened_tensor.max() + 1).int()
